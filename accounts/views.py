    ...
    if request.method == 'POST' and form.is_valid():
        if obj and obj.pk == request.user.pk and not form.cleaned_data['is_active']:
            ...
            with transaction.atomic():
                obj = form.save()
                log_action(request.user, 'User updated' if pk else 'User created', obj,
                           f'{obj.username}; active={obj.is_active}')
            messages.success(request, 'User saved successfully.')
            return redirect('users')
    return render(request, 'form.html',
                  {'title': 'Edit user' if pk else 'Create user', 'form': form, 'cancel_url': 'users'})
...
def profile(request):
    ...
    return render(request, 'profile.html',
                  {'title': 'My profile', 'form': form, 'image_form': image_form, 'profile': obj})
...
def roles(request):
    return render(request, 'roles.html',
                  {'title': 'Roles & permissions', 'groups': Group.objects.prefetch_related('permissions')})
...
def role_edit(request, pk=None):
    ...
    return render(request, 'form.html',
                  {'title': 'Edit role' if pk else 'Create role', 'form': form, 'cancel_url': 'roles'})
