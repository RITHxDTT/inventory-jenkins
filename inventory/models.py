...
class Product(CatalogModel):
    ...
    supplier = models.ForeignKey('suppliers.Supplier', on_delete=models.PROTECT, related_name='products', null=True,
                                 blank=True)
    ...
    class Meta:
        ordering = ['name']
        constraints = [models.CheckConstraint(condition=Q(cost_price__gte=0) & Q(selling_price__gte=0),
                                              name='product_nonnegative_prices')]
        indexes = [models.Index(fields=['is_active', 'current_quantity'])]
        permissions = [('view_dashboard', 'Can view dashboard'), ('view_reports', 'Can view inventory reports'),
                       ('stock_in', 'Can receive stock'), ('stock_out', 'Can issue stock')]
    ...
    def save(self, *args, **kwargs):
        ...
        if not self._state.adding and kwargs.get('update_fields') is None:
            kwargs['update_fields'] = [field.name for field in self._meta.concrete_fields if
                                       not field.primary_key and field.name != 'current_quantity']
...
class StockTransaction(models.Model):
    ...
    class Meta:
        ...
        constraints = [models.CheckConstraint(condition=Q(quantity__gt=0), name='movement_positive_quantity'),
                       models.CheckConstraint(condition=(
                                   Q(transaction_type='IN', quantity_after=F('quantity_before') + F('quantity')) | Q(
                               transaction_type='OUT', quantity_after=F('quantity_before') - F('quantity'))),
                                              name='movement_balanced')]
