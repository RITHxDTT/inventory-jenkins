...
@transaction.atomic
def move_stock(*, product, quantity, transaction_type, user, supplier=None, reason='', reference_number='', note='', transaction_date=None):
    ...
    movement = StockTransaction.objects.create(product=product, transaction_type=transaction_type, quantity=quantity, quantity_before=before, quantity_after=product.current_quantity, supplier=supplier if transaction_type == 'IN' else None, reason=reason, reference_number=reference_number, note=note, performed_by=user, transaction_date=transaction_date or timezone.localdate())
